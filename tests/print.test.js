...
test('@page の下余白は 5〜8mm(紙の下端で欠けず、1枚に載る量も減らさない)', function () {
  // 2026-09-15、下余白を 0/5/8/9/10/15mm に振り、明細 1〜30行のA4のPDFを実際に出して測った。
  //
  //   下余白 | 1枚に収まる最大 | 1ページ目の本文の下端(最も下) | 紙の下端まで
  //   -------|-----------------|-------------------------------|-------------
  //      0mm |           13行  | 293.6mm(明細16行)            |  3.4mm ← 欠ける
  //      5mm |           13行  | 288.3mm                       |  8.7mm
  //      8mm |           13行  | 284.1mm(明細15行)            | 12.9mm
  //      9mm |           12行  |                               |        ← 明細13行が2枚になる
  //     15mm |           12行  |                               |        ← 同上
  //
  // 下限が 5mm なのは、0mm だと1ページ目の最後の行が紙の下端から 3.4mm しかなく、
  // 多くの家庭用・事務用プリンタが印字できない 4〜5mm の帯に入るためである
  // (2026-09-14 に直した2ページ目の上端と、同じ種類の欠陥だった)。
  // 上限が 8mm なのは、9mm 以上にすると明細13行の書類が1枚ぶん増えるためである。
  // 8mm では明細 1〜30行でページ数が1件も変わらず、白紙のページも出ない(実測)。
  // 送付状は1ページ・2ページとも変わらず、解説記事も4ページのままである(実測)。
  const bottom = mm(pageMargin.bottom);
  assert.ok(bottom >= 5, '@page の下余白が 5mm 未満(1ページ目の最後の行が紙の下端で欠ける): ' + pageMargin.bottom);
  assert.ok(bottom <= 8, '@page の下余白が 8mm 超(明細13行の書類が1枚ぶん増える): ' + pageMargin.bottom);
});
