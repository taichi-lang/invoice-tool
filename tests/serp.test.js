...
/*
 * 2026-09-19 に足した。09-18 のテストは「名乗った4書類が h2 に在るか」までを見たが、
 * その工程が置いたリンクの先が、説明文で約束した中身になっているかは1件も見ていなかった。
 *
 * 実際に6工程ぶん数えたところ、1件だけ約束が外れていた。
 *   工程3 の説明文は「見積書と同じ内容をそのまま流用できます。」だったが、
 *   リンク先 /nohinsho の本文に「流用」は0件で、h2 は
 *   出す・出さないの判断 / 支払期日 / 収入印紙 / 納品書兼請求書 / 番号 / 納品書から請求書をつくる
 *   の6つだった。つまり説明文は、そのページが答えない中身を約束していた。
 *
 * ⚠ 語を足させるためのテストではない。母屋の説明文はクリック前の文脈をつくる役目で、
 *    そこに書いた約束は着地先で回収できなければならない、という1点だけを守る。
 *    (ObsidianVault/60_ライティング知識/04_SEO/内部リンクを関連記事から集めて順位を上げる)
 */
const PROMISES = [
  ['/mitsumorisho', ['有効期限', 'ただし書き', '収入印紙']],
  ['/nohinsho', ['出す・出さない', '納品書兼請求書', '入力し直さない']],
  ['/', ['インボイス', '源泉徴収', '登録番号']],
  ['/soufujo', ['頭語', '結語', '同封']],
  ['/guide/gensen-choshu-keisan', ['税抜', '税込', '100万円']],
  ['/ryoshusho', ['上様', '品代', '但し書き']],
  ['/inshi', ['5万円', '区分']],
];

const FILE_BY_HREF = (href) => (href === '/' ? 'index.html' : href.slice(1) + '.html');

test('母屋の説明文が約束した語は、そのリンク先のページに実在する', () => {
  for (const [href, words] of PROMISES) {
    const target = read(FILE_BY_HREF(href));
    for (const w of words) {
      assert.ok(target.includes(w),
        '母屋が ' + href + ' について約束した「' + w + '」が、リンク先に0件');
    }
  }
});

test('母屋は、6工程のリンク先を1つも欠かさず置いている', () => {
  const hub = read(HUB);
  for (const [href] of PROMISES) {
    assert.ok(hub.includes('href="' + href + '"'),
      '母屋から ' + href + ' へのリンクが無い');
  }
});
