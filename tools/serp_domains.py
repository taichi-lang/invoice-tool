...
⚠ 2026-09-04 に分かった2件(どちらも測って分かったこと):
  1. curl からの取得は bot 判定(anomaly)に落ちる。アプリ内ブラウザの同一オリジン fetch で測ること。
  2. **同じ語を続けて投げても、返る10件の顔ぶれと並びが毎回ちがう。**
     「請求書 書き方 必須項目 インボイス」で自サイトは 10位/7位/10位/8位/7位 と揺れた。
     1回だけ測って「圏外」と書くと、実際には載っている語を落とす(09-02 に実際に落とした)。
     **1語につき最低3回測り、3回とも出なかったときだけ「圏外」と書く。**
"""
